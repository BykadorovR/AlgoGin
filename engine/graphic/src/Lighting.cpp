...
void Lighting::Reposition(Shader* s)
{
	Shader* shader = s;
	shader->useProgram();
	GLuint program = shader->getProgram();
...
{
	omniLightSources.push_back(new OmniLight(col, pos));
...
{
	directLightSources.push_back(new DirectLight(col, dir));
	directLightSources[directLightSources.size() - 1]->SetShadows(shadow);
}

OmniLight* Lighting::GetOmniLightSource(int i)
{
	return omniLightSources[i];
}

DirectLight* Lighting::GetDirectLightSource(int i)
{
